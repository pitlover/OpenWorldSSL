'''
This code is orignally from https://www.lamda.nju.edu.cn/code_NACH.ashx
'''
...
if __name__ == '__main__':
    ...
    for i, folder_name in enumerate(folders):
        ...
        for extension in IMG_EXTENSIONS:
            files.extend(glob.glob(os.path.join('../Datasets/imagenet/train', folder_name, '*' + extension)))

        for filename in files:
            if i < cls_num and np.random.rand() < ratio:
                fout_train_label.write('train/%s/%s %d\n' % (folder_name, filename[37:], i))
            else:
                fout_train_unlabel.write('train/%s/%s %d\n' % (folder_name, filename[37:], i))
